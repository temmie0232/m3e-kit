...
### ★`data-theme` / `data-seed` は予約★

この2つの属性は「その要素から下の配色を切り替える」という意味を持つ。
**アプリのデータ属性として使ってはいけない。**

いちばん踏みやすいのがテーマ切替ボタン:

```html
<!-- ✗ そのボタンだけダーク配色になる -->
<button class="seg__btn" data-theme="dark">ダーク</button>

<!-- ✓ 別の名前にする -->
<button class="seg__btn" data-mode="dark">ダーク</button>
```

✗ のほうは、ボタン自身の `--surface` / `--on-surface-variant` が
ダーク版に解決される。ライトの地の上で文字だけ薄い灰色になり、
**エラーも警告も出ないまま読めなくなる**（実測で踏んだ）。
