...
| ✅ 足す | 密度（`--density`）。ただし**詰めたあとも 48px が残るものだけ** |
| ❌ 足さない | タップ判定を 48px 未満にする |
...
幅が決めるのは「レイアウトが何枚か」だけ。「指かマウスか」は幅では決まらない。
`min-width` で密度を切り替えると、こうなる:

- PC でウィンドウを半分にした瞬間、マウスは変わっていないのに携帯の寸法に戻る
- 大きいタブレット（指で触る）が PC 扱いになり、**指が当たらなくなる**

```css
/* tokens.css */
:root {
  --density: 0px;          /* 既定＝触る画面。M3 のタッチ用の寸法そのまま */
}

@media (pointer: fine) {
  :root {
    --density: 8px;        /* M3 density -2 相当。4px 刻みで詰める */
  }
}
```
...
```css
.search { min-height: calc(56px - var(--density)); }   /* 56 → 48 */
```

### 掛けてよいもの・いけないもの

`min-height` の値。実際の高さは中身のほうが高ければそちらが勝つ
（下の実測値は 1600px 幅の Chromium で測ったもの）。

| | 触る画面 | マウス | |
|---|---|---|---|
| `.search` | 56 | 48（実測 48） | ✅ |
| `.input` / `.textarea` / `.select` | 56 | 48（実測 50） | ✅ |
| `.row` | 56 | 48（実測 50） | ✅ |
| `.btn` | 48 | — | ❌ 詰めると 40 になる |
| `.menu__item` | 48 | — | ❌ 同上 |
| `.navbar__item` / `.rail__item` | 48 / 56 | — | ❌ 主要な行き先を細くしない |
| `.toolbar` | 64 | — | ❌ 掛けても**変わらない**（下記） |

`.toolbar` は中身が「48 のアイコンボタン + 上下 8 の余白」でちょうど 64 になる。
`min-height` を削っても中身が支えるので高さは 1px も動かない。
縮めるにはアイコンボタンを 48 未満にするしかなく、それは禁止。
**「掛けたのに効かない」宣言を残さないこと。**

**★48px を下回らせない★** タッチ対応のノート PC は
`pointer: fine` を返す（プライマリ入力がマウスのため）。その機械でも画面は
指で触れるので、`fine` を「指が来ない」と読んではいけない。
詰めてよいのは、詰めたあとも 48px が残る寸法だけ。
