...
- 左の索引で探す（`⌘K` で検索へ。日本語名・英語名・shadcn の名前・別名で引ける）
- プレビューは iframe。**幅を 360 / 720 / 1100 に切り替えられる**ので、
  PC のまま「携帯で崩れるか」と適応レイアウトを確認できる
- 上の帯でシード7色と明暗を切り替えられる
- 各部品に「守ること（★）」と、コピーできる HTML が付いている
- `#<id>` のリンクで直接開ける（例 `/demo/#seg`）

**新しく部品を足したら `demo/catalog.js` に1エントリ足す。**
目で確かめられない部品は壊れる。html は1箇所にしか書かない — 同じ文字列が
プレビューとコード欄の両方になるので、見本と貼り付けたコードは絶対にズレない。
